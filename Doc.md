...
[Kunde]:  https://raw.githubusercontent.com/Happy-Ferret/Virtasia/master/img/Kunde1.png?token=AB3yOmZ27O7nFJ-HYGP6MeB_MlkwkBKhks5Uh_s4wA%3D%3D "Installation"
...
####2. Aufgabenstellung und Planung
...
|Abschlussbesprechung          				      |2 Std                |                               
|Dokumentation und Erstellung der Präsentation 		      | 8 Std               | 
|**Gesamt**                                                   | **35 Std**          |
...
Im Anschluss erfolgt der Austausch des Kernels. Der genaue Vorgang hierfür ist dem Installationsskript ([Anlage 1](#Anlage1)) zu entnehmen.
...
Die Erzeugung einer kompletten grafischen Umgebung wird dank der Mischung aus Javascript, HTML5, XML und C/C++ stark vereinfacht. So besteht die Definition der kompletten Virtasia Benutzerumgebung ([Anlage 2](#Anlage2)) aus weit unter 100 Zeilen Quellcode. 
...
Virtasia ist ein fertiges Endprodukt, welches alle Softwarequalitätsstandards erfüllen sollte. 
Die Installation gestaltet sich daher recht einfach. 

1. Legen Sie das Installationsmedium in den Computer ein. 
2. Starten Sie den Computer neu. 
3. Das Installationsprogram wird nun gestartet. 
4. Folgen Sie den einzelnen Schritten der Installation. Während dieser werden Sie auch nach dem gewünschten root Passwort und der Systempartition gefragt. 
5. Nach der Installation wird der Computer automatisch neu gestartet. Virtasia ist nun Einsatzbereit. 
...
Zur Konfiguration und Installation eines Gastsystems gehen Sie wie folgt vor. 
...
![alt text][Kunde2]
5. Der Wizard führt Sie durch die komplette Konfiguration des Betriebssystems, an dessen Ende Sie ein gültiges Systemimage (.iso oder Installations-CD/DVD/USB) auswählen müssen.
6. Nach der Installation des Gastsystems, sollten Sie Virtualbox beenden. Der Hostcomputer startet sich dann automatisch neu.
7. Nach dem Neustart steht das neue Gastsystem unter dem, bei der Erstkonfiguration gewähltem, Namen zur Verfügung.
![alt text][Kunde3]
...
Normalerweise sind Kernel Updates nur selten von Nöten. Sollte es allerdings einmal an Treibern für neuere Hardware mangeln, lässt sich ein neuer Kernel mit aktuelleren Kernel Modulen wie folgt installieren.

1. Booten Sie den Computer neu.
2. Drücken Sie während der Anzeige des Bootsplashscreens ALT+F12, um in den Unix Single-User Modus zu gelangen. 
3. Geben Sie, auf Anfrage, das root Passwort ein. 
4. Sie befinden sich nun in der Shell des root Benutzers.
5. Navigieren Sie in das Verzeichnis kernel-update ("cd kernel-update").
6. Führen Sie, zum Herunterladen des gewünschten Kernels, das Skript download-kernel.sh aus ("./download-kernel.sh").
7. Geben Sie die Versionsnummer des gewünschten Kernels ein. 
8. Der gewünschte Kernel wird nun heruntergeladen. 
9. Nach dem erfolgreichen Download des Kernels, entpacken Sie diesen in das Arbeitsverzeichnis ("tar -xf linux*"
10. Es müssen nun Veränderungen an dem vorhandenen install.sh Skript vorgenommen werden. Hierfür empfählt sich die Verwendung von vi ("vi install.sh")
11. Schreiben Sie die bereits vorhandenen Aufrufzeilen auf die Versionsnummer des neuen Kernels um und speichern Sie das Skript erneut ab.
12. Führen Sie das Skript aus. 
13. Der Hostcomputer wird nun neugestartet. Nach Abschluss des Reboots steht der neue Kernel bereit und wird automatisch verwendet.
